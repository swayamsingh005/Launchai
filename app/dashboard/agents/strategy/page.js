  ...
  const runAgent = async () => {
    ...
    try {
      const res = await fetch("/api/strategy-agent", {
        ...
        body: JSON.stringify({
          idea: "AI-powered startup launcher",
          target_audience: "Early stage founders",
          goal: "Get first 10 paying clients",
          client_id: "test"
        })
      });
    ...
    <main style={{ minHeight: "100vh", background: "#08061a", color: "#f0eeff", fontFamily: "'Space Grotesk', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag { display: inline-block; font-size: 0.72rem; padding: 2px 8px; border-radius: 100px; background: rgba(124,58,237,0.15); color: #a78bfa; border: 1px solid rgba(124,58,237,0.2); margin-right: 0.4rem; margin-bottom: 0.4rem; }
      `}</style>
      ...
      <div className="content">
        <div className="agent-header">
          ...
          <div>
            <div className="agent-title">Strategy Agent</div>
            <div className="agent-desc">Generates 30-day launch plan, pricing strategy, target audience and weekly roadmap</div>
          </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            No activity yet — click "Run Agent" to generate your first strategy.<br />
