    ...
    <main style={{ minHeight: "100vh", background: "#08061a", color: "#f0eeff", fontFamily: "'Space Grotesk', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .info-box { background: rgba(124,58,237,0.08); border: 1px solid rgba(124,58,237,0.2); border-radius: 12px; padding: 0.75rem 1rem; margin-bottom: 1.5rem; font-size: 0.85rem; color: #a78bfa; line-height: 1.5; }
        ...
        .email-sent { display: inline-block; font-size: 0.7rem; padding: 2px 8px; border-radius: 100px; background: rgba(59,130,246,0.15); border: 1px solid rgba(59,130,246,0.3); color: #60a5fa; margin-left: 0.5rem; }
        .highlights { display: flex; flex-direction: column; gap: 0.4rem; margin-bottom: 0.75rem; }
        .highlight-item { font-size: 0.85rem; color: #d1d5db; line-height: 1.5; }
        ...
        .next-item { font-size: 0.85rem; color: #a78bfa; padding: 0.3rem 0; }
      ...
      <div className="content">
        <div className="agent-header">
          ...
          <div>
            <div className="agent-title">Report Agent</div>
            <div className="agent-desc">Sends weekly P&L summary every Monday — to you and each client via Resend</div>
          </div>
          <button className="run-btn" onClick={runAgent} disabled={running}>
            {running ? "Sending..." : "▶ Send Report"}
          </button>
        </div>

        <div className="info-box">
          📧 Report will be sent to: <strong>{email}</strong> · Runs automatically every Monday 8AM IST once scheduled
        </div>

        <p className="section-title">Activity log — last 10 reports</p>

        {loading ? (
          <div style={{ color: "#6b7280", fontSize: "0.85rem" }}>Loading activity...</div>
        ) : outputs.length === 0 ? (
          <div className="empty-state">
            No reports sent yet — click "Send Report" to send your first weekly summary.<br />
            Report will be emailed to you via Resend automatically.
          </div>
        ) : (
          outputs.map(o => (
            <div className="output-card" key={o.id}>
              <div className="output-time">
                ...
                {o.output?.email_sent && <span className="email-sent">📧 Email sent</span>}
              </div>
              <div style={{ fontSize: "0.8rem", color: "#7c3aed", fontWeight: 600, marginBottom: "0.5rem" }}>{o.output?.week}</div>
              <div className="sub-label">Highlights</div>
              <div className="highlights">
                {(o.output?.highlights || []).map((h, i) => (
                  <div className="highlight-item" key={i}>✅ {h}</div>
                ))}
              </div>
              <div className="sub-label">Next Week Focus</div>
              {(o.output?.next_week_focus || []).map((f, i) => (
                <div className="next-item" key={i}>→ {f}</div>
              ))}
            </div>
          ))
        )}
