  ...
  const runAgent = async () => {
    ...
    try {
      const res = await fetch("/api/support-agent", {
        ...
        body: JSON.stringify({
          question,
          product_info: "LaunchAI is an AI-powered platform that gives founders 12 AI agents working 24/7 for their startup.",
          client_id: "test"
        })
      });
      const data = await res.json();
      if (data.success) { await fetchOutputs(); setQuestion(""); }
    ...
    <main style={{ minHeight: "100vh", background: "#08061a", color: "#f0eeff", fontFamily: "'Space Grotesk', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .question-input { flex: 1; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #f0eeff; padding: 0.75rem 1rem; border-radius: 10px; font-size: 0.88rem; font-family: 'Space Grotesk', sans-serif; outline: none; }
        .question-input:focus { border-color: rgba(124,58,237,0.5); }
        ...
        .question-label { font-size: 0.75rem; color: #7c3aed; font-weight: 600; margin-bottom: 0.4rem; text-transform: uppercase; letter-spacing: 0.08em; }
        .answer-label { font-size: 0.75rem; color: #4ade80; font-weight: 600; margin: 0.75rem 0 0.4rem; text-transform: uppercase; letter-spacing: 0.08em; }
        .output-content { font-size: 0.85rem; color: #d1d5db; line-height: 1.7; }
        ...
        .escalate-badge { display: inline-block; font-size: 0.7rem; padding: 2px 8px; border-radius: 100px; background: rgba(239,68,68,0.15); border: 1px solid rgba(239,68,68,0.3); color: #f87171; margin-left: 0.5rem; }
      `}</style>
      ...
      <div className="content">
        <div className="agent-header">
          ...
          <div>
            ...
            <div className="agent-desc">Answers customer queries 24/7 — trained on your product info</div>
          </div>
        </div>
        ...
        <div className="input-row">
          <input
            ...
            placeholder="Type a customer question to test e.g. How does LaunchAI work?"
            value={question}
            onChange={e => setQuestion(e.target.value)}
            onKeyDown={e => e.key === "Enter" && runAgent()}
          />
          <button className="run-btn" onClick={runAgent} disabled={running || !question.trim()}>
            {running ? "Answering..." : "▶ Answer"}
          </button>
        </div>

        <p className="section-title">Activity log — last 10 queries</p>

        {loading ? (
          <div style={{ color: "#6b7280", fontSize: "0.85rem" }}>Loading activity...</div>
        ) : outputs.length === 0 ? (
          <div className="empty-state">
            No activity yet — type a customer question above to test.<br />
            Once Anthropic credits are added, answers will be personalised using Claude AI.
          </div>
        ) : (
          outputs.map(o => (
            <div className="output-card" key={o.id}>
              <div className="output-time">
                ...
                {o.output?.escalate_to_founder && <span className="escalate-badge">⚠ Escalate to founder</span>}
              </div>
              <div className="question-label">Question</div>
              <div className="output-content">{o.output?.question || "—"}</div>
              <div className="answer-label">Answer</div>
              <div className="output-content">{o.output?.answer || JSON.stringify(o.output, null, 2)}</div>
            </div>
          ))
        )}
      </div>
