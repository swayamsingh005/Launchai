  ...
  const runAgent = async () => {
    ...
    try {
      const res = await fetch("/api/code-agent", {
        ...
        body: JSON.stringify({ task, tech_stack: "Next.js, Supabase", client_id: "test" })
      });
      const data = await res.json();
      if (data.success) { await fetchOutputs(); setTask(""); }
    ...
    <main style={{ minHeight: "100vh", background: "#08061a", color: "#f0eeff", fontFamily: "'Space Grotesk', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .task-input { flex: 1; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #f0eeff; padding: 0.75rem 1rem; border-radius: 10px; font-size: 0.88rem; font-family: 'Space Grotesk', sans-serif; outline: none; }
        .task-input:focus { border-color: rgba(124,58,237,0.5); }
        .task-input::placeholder { color: #4b5563; }
        .run-btn { background: linear-gradient(135deg, #7c3aed, #5b21b6); border: none; color: #fff; padding: 0.6rem 1.5rem; border-radius: 10px; font-size: 0.9rem; cursor: pointer; font-family: 'Space Grotesk', sans-serif; font-weight: 500; transition: all 0.2s; white-space: nowrap; }
      ...
      <div className="content">
        <div className="agent-header">
          ...
          <div>
            <div className="agent-title">Code Agent</div>
            <div className="agent-desc">Writes and reviews code for your client's product — features, bug fixes, improvements</div>
          </div>
        </div>
        ...
        <div className="input-row">
          <input
            ...
            placeholder="Describe what to build e.g. Add a contact form with email validation..."
            value={task}
        ...
        {loading ? (
          ...
          <div className="empty-state">
            No activity yet — describe a coding task above and click Run.<br />
