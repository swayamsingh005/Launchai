...
export async function POST(request) {
  try {
    ...
    const limitCheck = await checkUsageLimit(supabase, user.id, client_id, "outreach");
    ...
    await logUsage(supabase, user.id, client_id, "outreach");

    return NextResponse.json({ success: true, outreach: savedOutreach, runsRemaining: limitCheck.remaining - 1 });
  } catch (err) {
